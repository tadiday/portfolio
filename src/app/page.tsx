...
function RisingBlock({ progress, block, index, marker }: { progress: MotionValue<number>; block: typeof risingBlocks[number]; index: number; marker?: { number: string; label: string } }) {
  const y = useTransform(progress, [0, block.start, block.end], ["100%", "100%", "0%"]);
  ...
  return (
    ...
    >
      ...
      <span className="absolute right-2 top-2 font-mono text-[8px] font-bold text-[var(--home-accent)]">0{index + 1}</span>
      {index === 2 && marker && <div className="absolute left-1/2 top-4 flex h-8 w-[230px] -translate-x-1/2 items-center justify-center font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-[var(--home-accent)]">
        ...
        <span>{"// "}{marker.number} — {marker.label}</span>
      </div>}
    </motion.span>
  );
}
...
function TransitionSection({ children, id, number, label, className = "" }: { children: ReactNode; id?: string; number: string; label: string; className?: string }) {
  const ref = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start end", "end start"] });

  return <div ref={ref} id={id} className={`relative z-30 grid ${className}`}>
    <BrutalistSectionTransition progress={scrollYProgress} number={number} label={label} />
    {children}
  </div>;
}
