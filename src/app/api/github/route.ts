...
const levels: Record<GitHubLevel, ActivityLevel> = {
  NONE: 0,
  FIRST_QUARTILE: 1,
  SECOND_QUARTILE: 2,
  THIRD_QUARTILE: 3,
  ...
  try {
    const response = await fetch("https://api.github.com/graphql", {
      ...
      headers: {
        ...
        "User-Agent": "tadiday-portfolio",
      },
      body: JSON.stringify({ query, variables: { login: "tadiday" } }),
    ...
    if (result.errors?.length || !calendar || !user)
      return NextResponse.json(
        { error: "GitHub returned no contribution data." },
        { status: 502 },
      );
    ...
    repositories.forEach((repository) =>
      repository.languages.edges.forEach(({ node, size }) =>
        languageBytes.set(
          node.name,
          (languageBytes.get(node.name) ?? 0) + size,
        ),
      ),
    );
    const totalLanguageBytes = Array.from(languageBytes.values()).reduce(
      (sum, size) => sum + size,
      0,
    );
    const topLanguages = Array.from(languageBytes, ([name, size]) => ({
      name,
      percentage: totalLanguageBytes
        ? Math.round((size / totalLanguageBytes) * 100)
        : 0,
    }))
