...
import { ArrowLeft, ArrowRight, ExternalLink, FolderGit2, Gauge, GitBranch, Globe2, ReceiptText, UtensilsCrossed, X } from "lucide-react";
...
function Label({ children }: { children: ReactNode }) {
  return <h3 className="font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--home-accent)]">{"// "}{children}</h3>;
}

type ProjectItem = typeof projects[number];

function ProjectModal({ project, onClose }: { project: ProjectItem; onClose: () => void }) {
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-black/85 p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="project-modal-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="relative max-h-[92vh] w-full max-w-[1280px] overflow-y-auto border border-[#666b70] bg-[#08090a] p-5 text-white sm:p-7"><Corners /><button type="button" onClick={onClose} aria-label="Close project details" className="absolute right-3 top-3 z-10 grid h-9 w-9 place-items-center border border-[#666b70] bg-[#08090a] hover:border-[var(--home-accent)] hover:text-[var(--home-accent)]"><X className="h-5 w-5" /></button>
    ...
    <motion.main className="mx-auto w-full max-w-[1440px] space-y-3" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.08 }} transition={{ duration: 0.55 }}>
      <section className="grid gap-3 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,.95fr)] xl:grid-cols-[minmax(0,1.5fr)_minmax(0,.7fr)_minmax(0,.7fr)]">
        <div className="relative flex min-h-[205px] flex-col justify-center px-4 py-7 sm:px-8 xl:flex-row xl:items-center xl:gap-10">
          ...
          <div className="mt-5 max-w-[34ch] xl:mt-0"><p className="flex items-center gap-3 font-mono text-xs uppercase tracking-[0.18em] text-[var(--home-accent)]">Things I&apos;ve built <span className="text-lg text-[#858a90]">+</span></p><p className="mt-4 font-mono text-[11px] font-medium leading-5 text-[#c4c7ca]">A collection of personal projects and experiments. Each one taught me something new and pushed me to grow as a developer.</p></div>
        </div>

        <Panel className="p-5 lg:p-6"><Label>Filter projects</Label><div className="mt-4 space-y-1">{filters.map((item) => { const count = item === "All projects" ? projects.length : projects.filter((project) => project.category === item).length; return <button key={item} type="button" onClick={() => chooseFilter(item)} className={`flex w-full items-center justify-between px-3 py-2 font-mono text-[9px] font-semibold uppercase transition-colors ${filter === item ? "bg-[var(--home-accent)] text-[#08090a]" : "text-[#d0d3d6] hover:bg-white/5"}`}><span>{item === "All projects" ? item : `${item}s`}</span><span>{String(count).padStart(2, "0")}</span></button>; })}</div></Panel>

        <Panel className="p-5 lg:col-span-2 lg:p-6 xl:col-span-1"><Label>Technologies used</Label><div className="mt-5 flex flex-wrap gap-2">{["React.js", "Vue 3", "Node.js", "TypeScript", "Python", "Express", "MySQL", "Docker", "Git", "+ More"].map((tech) => <span key={tech} className="border border-[#3d4146] px-3 py-1.5 font-mono text-[9px] font-semibold uppercase text-[#c8cbce]">{tech}</span>)}</div></Panel>
      </section>

      <Panel className="p-4 sm:p-5"><Label>{filter}</Label><div className="mt-4 divide-y divide-white/10 lg:h-[510px]">
        {visible.map((project, index) => <article key={project.title} role="button" tabIndex={0} onClick={() => setSelected(project)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelected(project); }} className="grid cursor-pointer gap-4 py-5 outline-none transition-colors first:pt-2 hover:bg-white/[0.02] focus-visible:bg-white/[0.04] lg:grid-cols-[36px_190px_minmax(0,1fr)_190px] lg:items-center">
          <span className="font-mono text-[11px] font-bold text-[#d8dade]">{String(page * 3 + index + 1).padStart(2, "0")}</span>
          <div className="relative grid min-h-[126px] place-items-center border border-[#4b5055] text-[var(--home-accent)]"><project.Icon className="h-10 w-10" strokeWidth={1.5} aria-hidden="true" /><span className="absolute left-1.5 top-1.5 h-2 w-2 border-l border-t border-white/55" /><span className="absolute bottom-1.5 right-1.5 h-2 w-2 border-b border-r border-white/55" /></div>
          <div className="min-w-0 lg:px-2"><h3 className="text-lg font-bold text-white">{project.title}</h3><p className="mt-1 font-mono text-[9px] font-semibold uppercase text-[var(--home-accent)]">{project.category}</p><p className="mt-3 max-w-[64ch] text-xs font-medium leading-5 text-[#c6c9cc]">{project.description}</p><div className="mt-3 flex flex-wrap gap-2">{project.tech.map((tech) => <span key={tech} className="border border-[#3d4146] px-2 py-1 font-mono text-[8px] font-semibold uppercase text-[#c8cbce]">{tech}</span>)}</div></div>
          <div className="border-white/10 font-mono uppercase lg:border-l lg:pl-6"><p className="text-[10px] font-bold text-white">{project.date}</p><p className="mt-2 text-[9px] font-semibold text-[var(--home-accent)]">{project.kind}</p><div className="mt-6 space-y-3"><span className="flex items-center gap-2 text-[10px] font-semibold text-[var(--home-accent)]">View details <ArrowRight className="h-3.5 w-3.5" /></span>{project.github ? <a onClick={(event) => event.stopPropagation()} href={project.github} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-[10px] font-semibold text-white hover:text-[var(--home-accent)]">GitHub <ExternalLink className="h-3.5 w-3.5" /></a> : <span className="flex items-center gap-2 text-[9px] text-[#7f858c]"><GitBranch className="h-3.5 w-3.5" />Private / pending</span>}</div></div>
        </article>)}
      </div></Panel>

      <Panel className="flex items-center justify-between px-5 py-3 font-mono text-[10px] font-semibold uppercase"><button type="button" onClick={() => setPage((current) => Math.max(0, current - 1))} disabled={page === 0} className="flex items-center gap-2 text-[var(--home-accent)] disabled:opacity-25"><ArrowLeft className="h-4 w-4" />Prev</button><span className="text-white">{String(page + 1).padStart(2, "0")} / {String(pageCount).padStart(2, "0")}</span><button type="button" onClick={() => setPage((current) => Math.min(pageCount - 1, current + 1))} disabled={page >= pageCount - 1} className="flex items-center gap-2 text-[var(--home-accent)] disabled:opacity-25">Next<ArrowRight className="h-4 w-4" /></button></Panel>
    </motion.main>
