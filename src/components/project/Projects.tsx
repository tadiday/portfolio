  ...
  return <Panel className="mt-8 p-5 sm:p-6">
    <div className="flex flex-col justify-between gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-center">
      <div className="flex items-center gap-3">
        ...
        <span className="font-mono text-[10px] font-bold text-[#858b91]">({String(archivedProjects.length).padStart(2, "0")})</span>
      </div>
      <p className="font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-[#858b91]">Earlier work / kept for the record</p>
    </div>

    <div className="divide-y divide-white/10">
      {archivedProjects.map((project, index) => <a key={project.title} href={project.href} target="_blank" rel="noreferrer" className="group grid gap-3 py-5 transition-colors hover:bg-white/[0.025] sm:grid-cols-[50px_minmax(0,1fr)_150px_65px] sm:items-center sm:px-2">
        <span className="font-mono text-[11px] font-bold text-[#858b91]">{String(index + 1).padStart(2, "0")}</span>
        <div className="min-w-0">
          <h3 className="text-lg font-bold text-[#d7dadd] transition-colors group-hover:text-white">{project.title}</h3>
        </div>
        <span className="font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-[#9ba0a6]">{project.category}</span>
        <span className="flex items-center justify-between gap-3 font-mono text-[10px] font-bold text-[#9ba0a6] sm:justify-end">{project.year}<ExternalLink className="h-3.5 w-3.5 text-[var(--home-accent)] transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></span>
      </a>)}
    </div>
  </Panel>;
}
    ...
    <motion.main className="mx-auto w-full max-w-[1440px] space-y-3" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.08 }} transition={{ duration: 0.55 }}>
      ...
      <Panel className="p-4 sm:p-6"><div className="flex items-center justify-between gap-4"><Label>{filter}</Label><div className="flex items-center gap-3 font-mono text-[10px] font-bold uppercase"><button type="button" onClick={() => setPage((current) => Math.max(0, current - 1))} disabled={page === 0} aria-label="Previous project page" className="grid h-8 w-8 place-items-center border border-[#3d4146] text-[var(--home-accent)] disabled:opacity-25"><ArrowLeft className="h-4 w-4" /></button><span className="text-white">{String(page + 1).padStart(2, "0")} / {String(pageCount).padStart(2, "0")}</span><button type="button" onClick={() => setPage((current) => Math.min(pageCount - 1, current + 1))} disabled={page >= pageCount - 1} aria-label="Next project page" className="grid h-8 w-8 place-items-center border border-[#3d4146] text-[var(--home-accent)] disabled:opacity-25"><ArrowRight className="h-4 w-4" /></button></div></div><div className="mt-4 divide-y divide-white/10 lg:h-[500px]">
        {visible.map((project, index) => <article key={project.title} role="button" tabIndex={0} onClick={() => setSelected(project)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelected(project); }} className="group relative grid min-h-[140px] cursor-pointer gap-5 py-4 outline-none transition-colors hover:bg-white/[0.02] focus-visible:bg-white/[0.04] lg:grid-cols-[44px_minmax(0,1fr)_210px] lg:items-center">
          ...
          <div className="min-w-0 lg:px-3"><h3 className="text-[26px] font-bold text-white">{project.title}</h3><p className="mt-3 max-w-[82ch] text-[15px] font-medium leading-6 text-[#d5d7da]">{project.description}</p><div className="mt-3 flex flex-wrap gap-2">{project.tech.map((tech) => <span key={tech} className="border border-[#3d4146] px-2.5 py-1 font-mono text-[10px] font-bold uppercase text-[#dde0e2]">{tech}</span>)}</div></div>
          <div className="border-white/10 font-mono uppercase lg:border-l lg:py-2 lg:pl-7"><p className="text-[13px] font-bold text-white">{project.date}</p><p className="mt-2 text-[11px] font-bold text-[var(--home-accent)]">{project.category}</p><div className="mt-5">{project.github ? <a onClick={(event) => event.stopPropagation()} href={project.github} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-[12px] font-bold text-white hover:text-[var(--home-accent)]">GitHub <ExternalLink className="h-4 w-4" /></a> : <span className="flex items-center gap-2 text-[11px] font-semibold text-[#aeb3b8]"><GitBranch className="h-4 w-4" />Private / pending</span>}</div></div>
        </article>)}
      </div></Panel>
