...
function getExperienceGroup(
  type: ExperienceKind,
): Exclude<ExperienceFilter, "All experience"> {
  return type === "Research" || type === "Education"
    ? "Academic"
    : "Professional";
}
...
function getExperienceYear(experience: ExperienceItem) {
  const yearSource =
    experience.end === "Present" ? experience.start : experience.end;
...
function ExperienceFilters({
  ...
  return (
    <DashboardPanel className="p-5">
      <DashboardLabel className="text-xs font-bold">
        Filter experience
      </DashboardLabel>
        ...
        {experienceFilters.map((filter) => {
          const count =
            filter === "All experience"
              ...
              : experiences.filter(
                  (experience) =>
                    getExperienceGroup(experience.type) === filter,
                ).length;
    ...
    >
      <div className="overflow-hidden">
        <div className="border-t border-[var(--border-muted)] pt-5">
          <DashboardLabel className="mb-3 text-[10px] font-bold">
            Contributions
          </DashboardLabel>
          <ul className="space-y-2">
            {experience.bullets.map((bullet) => (
              <li
                key={bullet}
                className="flex gap-3 text-[13px] font-medium leading-5 text-[var(--text-secondary)]"
              >
...
function ExperienceCard({
  ...
  const categoryLabel =
    experience.type === "Co-op" || experience.type === "Internship"
      ? "Internship"
      : experience.type;
  ...
  return (
    ...
    >
      ...
      />
      <span
        className="absolute -left-[41px] top-[43px] hidden h-px w-[41px] -translate-y-1/2 bg-[#4b5055] lg:block"
        aria-hidden="true"
      />
      ...
      <DashboardPanel className="p-4">
        <div className="grid gap-3 lg:grid-cols-[165px_minmax(0,1fr)_180px] lg:items-center lg:gap-5">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[10px] font-bold uppercase tracking-[0.05em]">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 text-[var(--home-accent)] hover:text-[var(--text-primary)]"
              >
                {experience.company}
          ...
          <div className="flex items-center justify-between gap-4 border-[var(--border-muted)] font-mono uppercase lg:border-l lg:pl-5">
            <span className="inline-block border border-[#4b5055] px-3 py-2 text-[10px] font-bold text-[var(--text-primary)]">
              {categoryLabel}
            </span>
            <button
              type="button"
...
export default function Experience() {
  ...
  const experiencesByYear = useMemo(() => {
    return filteredExperiences.reduce<
      Array<{ year: string; items: ExperienceItem[] }>
    >((groups, experience) => {
    ...
    >
      ...
      >
        ...
        <div className="relative mt-3 lg:pl-20">
          <div
            className="absolute bottom-[43px] left-8 top-[18px] hidden w-px bg-[#4b5055] lg:block"
            aria-hidden="true"
          />
          <div className="space-y-12">
            {experiencesByYear.map((group, groupIndex) => (
              <section
                key={group.year}
                aria-labelledby={`experience-year-${group.year}`}
              >
                <div className="relative mb-4 flex items-center gap-4">
                  <span
                    className="absolute -left-14 top-1/2 hidden h-4 w-4 -translate-y-1/2 border border-[var(--accent)] bg-[var(--background)] lg:block"
                    aria-hidden="true"
                  />
                  <h3
                  ...
                  <span
                    className="h-px flex-1 bg-gradient-to-r from-[#4b5055] to-transparent"
                    aria-hidden="true"
                  />
