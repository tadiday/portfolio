...
const contactLinks = [
  {
    label: "Email",
    value: "petercao49@gmail.com",
    href: "mailto:petercao49@gmail.com",
    Icon: Mail,
  },
  {
    label: "LinkedIn",
    value: "linkedin.com/in/petercao03",
    href: "https://www.linkedin.com/in/petercao03",
    Icon: Linkedin,
  },
  {
    label: "GitHub",
    value: "github.com/tadiday",
...
const inputClassName =
  "w-full border border-[var(--border)] bg-[var(--background)] px-3 py-3 font-mono text-[13px] text-[var(--text)] outline-none transition-colors placeholder:text-[var(--text-muted)] focus:border-[var(--accent)]";
  ...
  return (
    <div className="relative min-w-0 overflow-hidden px-3 py-5 sm:px-7 sm:py-8">
      ...
      <p className="mt-5 max-w-[52ch] font-mono text-[13px] font-medium leading-6 text-[var(--text-secondary)]">
        ...
        ideas involving software and technology.
      </p>
    </div>
  );
}
...
function ContactDirectory() {
  return (
    <DashboardPanel className="p-4 sm:p-5">
      <DashboardLabel className="mb-3 text-[11px] font-bold">
        Get in touch
      </DashboardLabel>
      <div>
        ...
        <div className="grid grid-cols-[36px_70px_minmax(0,1fr)] items-center gap-3 border-t border-[var(--border-muted)] py-2.5">
          ...
          <span className="font-mono text-[10px] leading-4 text-[var(--text-secondary)]">
            Washington, DC
            <br />
            United States
          </span>
...
function ContactForm() {
  ...
  return (
    <DashboardPanel className="h-full p-5 sm:p-7">
      <DashboardLabel className="mb-7 text-[11px] font-bold">
        Send a message
      </DashboardLabel>
      <form onSubmit={submitForm} className="grid gap-5">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button
            type="submit"
            disabled={sending}
            className="inline-flex min-w-[175px] items-center justify-between gap-6 border border-[var(--text-primary)] bg-[var(--text-primary)] px-4 py-3 font-mono text-[11px] font-black uppercase text-[var(--on-accent)] transition-colors hover:border-[var(--home-accent)] hover:bg-[var(--home-accent)] disabled:cursor-wait disabled:opacity-60"
          >
          ...
          <p
            aria-live="polite"
            className="font-mono text-[12px] text-[var(--home-accent)]"
          >
            {status}
          </p>
        </div>
...
function AvailabilityPanel() {
  return (
    <DashboardPanel className="mt-3 p-5">
      <DashboardLabel className="mb-5 text-[11px] font-bold">
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-[repeat(4,minmax(0,1fr))_180px]">
        {availability.map((item) => (
          <div
            key={item.label}
            className="grid grid-cols-[8px_1fr] gap-3 border-[var(--border-muted)] xl:border-r xl:pr-4"
          >
            <span className="mt-1 h-2 w-2 bg-[#35d07f] shadow-[0_0_8px_rgba(53,208,127,.45)]" />
            <div>
              <p className="font-mono text-[10px] font-black uppercase text-[var(--text-primary)]">
                {item.label}
              </p>
...
export default function Contact() {
  return (
    <section
      id="contact"
      className="relative z-30 overflow-hidden bg-[var(--background)] px-4 pb-8 pt-[calc(var(--home-header-height)+32px)] text-[var(--text)] sm:px-6 lg:px-8"
    >
      <motion.div
      ...
      >
        ...
        <AvailabilityPanel />
        <div className="mt-3 flex flex-wrap items-center justify-between gap-4 border border-[var(--border-strong)] bg-[var(--surface)] px-5 py-4 font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--text-muted)]">
          <p>Design &amp; build by Peter Cao / 2026</p>
          <button
            type="button"
            onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
            className="inline-flex items-center gap-3 text-[var(--text-primary)] hover:text-[var(--home-accent)]"
          >
